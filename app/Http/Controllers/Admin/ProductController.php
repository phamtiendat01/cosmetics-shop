    ...
    public function store(StoreProductRequest $request)
    {
        ...
        try {
            ...
            foreach ($variants as $v) {
                ...
                // ✅ Auto-mint certificate cho variant mới
                try {
                    $this->certificateService->mintCertificate($pv);
                } catch (\Exception $e) {
                    // Log error nhưng không block việc tạo sản phẩm
    ...
    public function edit(Product $product)
    {
        $product->load([
            ...
            'variants.adjustments' => function($q) {
                $q->latest()->limit(5); // show 5 log gần nhất
            }
        ]);
            ...
            foreach ($variants as $v) {
                if (!empty($v['id'])) {
                    ...
                    try {
                        ...
                        // Log error nhưng không block việc cập nhật sản phẩm
                        Log::warning('Failed to mint certificate for new variant', [
                            'variant_id' => $pv->id,
                            'error' => $e->getMessage(),
                        ]);
