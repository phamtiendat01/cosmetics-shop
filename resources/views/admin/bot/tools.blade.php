...
{{-- Stats Cards - Đồng bộ style admin --}}
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <div class="text-2xl font-bold text-slate-900 mb-1">{{ $tools->total() }}</div>
        <div class="text-sm text-slate-600">Tổng câu hỏi</div>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <div class="text-2xl font-bold text-green-600 mb-1">{{ $tools->where('is_active', true)->count() }}</div>
        <div class="text-sm text-slate-600">Đang hoạt động</div>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <div class="text-2xl font-bold text-slate-500 mb-1">{{ $tools->where('is_active', false)->count() }}</div>
        <div class="text-sm text-slate-600">Đã tắt</div>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
        <div class="text-2xl font-bold text-rose-600 mb-1">{{ $tools->pluck('category')->unique()->count() }}</div>
        <div class="text-sm text-slate-600">Danh mục</div>
    </div>
</div>

{{-- Table - Đồng bộ style admin --}}
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($tools->sortBy('order') as $tool)
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            @endif
                            <div>
                                <div class="font-medium text-slate-900">{{ $tool->question ?? 'Chưa có câu hỏi' }}</div>
                                <div class="text-xs text-slate-500 font-mono mt-1">{{ $tool->name }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-slate-700 max-w-md line-clamp-2">{{ Str::limit($tool->answer ?? 'Chưa có câu trả lời', 100) }}</div>
                    </td>
                    <td class="px-6 py-4">
                        @php
                        $categoryColors = [
                        ...
                        <span class="px-2 py-1 {{ $color }} rounded text-xs font-semibold">
                            {{ ucfirst($tool->category ?? 'general') }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <span class="text-sm text-slate-600">{{ $tool->order ?? 0 }}</span>
                    </td>
                    <td class="px-6 py-4">
                        @if($tool->is_active)
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <button onclick="editTool({{ json_encode($tool) }})"
...
<div id="toolModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <form method="POST" action="{{ route('admin.bot.tools.store') }}" class="p-6">
            @csrf
