...
{{-- Redesigned theo chuẩn Shopee/Lazada với RAG support --}}
<div x-data="cosmeBotWidget()" x-init="init()"
    class="fixed z-[9999] select-none"
    :style="`left:${x}px; top:${y}px`"
    style="touch-action:none">

    {{-- Bong bóng mascot với vầng hào quang và khuôn mặt cười --}}
    <button type="button"
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden h-full flex flex-col">
            ...
            {{-- Header (drag handle) - Đồng bộ với style admin --}}
            ...
            {{-- Content - Đồng bộ với style admin --}}
            <div id="botScroll"
                class="flex-1 overflow-y-auto px-4 py-4 space-y-3 text-sm bg-slate-50"
                style="scroll-behavior: smooth;">

                {{-- Welcome message với Quick Questions - Đồng bộ style admin --}}
                <div x-show="messages.length === 0" x-transition
                    ...
                    {{-- Quick Questions từ Tools - Enhanced UI chuyên nghiệp --}}
                    <div x-show="Object.keys(quickQuestions).length > 0"
                        ...
                        {{-- Header với divider - Đồng bộ style admin --}}
                        ...
                        <div class="space-y-4 max-h-[400px] overflow-y-auto pr-1" style="scrollbar-width: thin; scrollbar-color: rgba(244, 63, 94, 0.3) transparent;">
                            <template x-for="(category, catName) in quickQuestions" :key="catName">
                                <div class="space-y-2.5 px-2"
                                    ...
                                    {{-- Category Header - Đồng bộ style admin --}}
                                    ...
                                    {{-- Questions List - Full width buttons --}}
                                    <div class="space-y-2">
                                        <template x-for="item in category" :key="item.id">
                                            <button
                                                ...
                                                x-transition:enter-end="opacity-100 translate-y-0">
                                                {{-- Icon với background - Đồng bộ style admin --}}
                ...
                <template x-for="m in messages" :key="m.id">
                    <div :class="m.role==='user' ? 'flex justify-end' : 'flex justify-start'">
                        <div class="max-w-[85%]">
                            {{-- User message - Đồng bộ style admin --}}
                            ...
                            {{-- Bot message - Đồng bộ style admin --}}
                            ...
                            {{-- Quick reply chips - Chỉ 2 nút: Tư vấn mỹ phẩm và Reset (có icon) --}}
                            ...
                            {{-- Product cards - Enhanced Shopee/Lazada style --}}
                ...
                {{-- Quick Questions luôn hiển thị ở cuối (khi có messages) - Collapsible - Đồng bộ style admin --}}
    ...
    function cosmeBotWidget() {
        return {
            ...
            async loadQuickQuestions() {
                try {
                    const res = await fetch("{{ route('bot.tools') }}", {
                        headers: {
