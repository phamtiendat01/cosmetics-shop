{{-- resources/views/components/livechat/widget.blade.php --}}
<style>
    ...
    <div x-show="open" x-transition.opacity.scale.origin-bottom-right
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl border border-rose-100 overflow-hidden w-full h-full"
            ...
            <div class="p-3 border-t text-sm text-center bg-white">
                Vui lòng <a href="{{ route('login') }}" class="text-rose-600 underline">đăng nhập</a> để chat.
            </div>
            @endauth
    ...
    function liveChatWidget() {
        ...
        return {
            ...
            async init() {
                ...
                @auth
                try {
                    const r = await fetch(@json(route('livechat.unread')), {
                        headers: {
            ...
            async startChat() {
                const r = await fetch(@json(route('livechat.start')), {
                    method: 'POST',
                    headers: {
                ...
                if (window.Echo) {
                    @auth window.Echo.private(`chat.${this.chatId}`).listen('.message.sent', (e) => this.onReceive(e));
                    @else window.Echo.channel(`public-chat.${this.publicToken}`).listen('.message.sent', (e) => this.onReceive(e));
                    @endauth
                }
            },
            async fetchMessages() {
