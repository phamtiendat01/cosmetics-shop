...
<div class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="flex items-start justify-between gap-6">
        ...
        <div class="text-right space-y-2">
            ...
            {{-- Trả hàng: thông báo hạn cuối + nút --}}
            <div class="text-xs mt-1">
                @if($okStatusForReturn && $completedAt)
                @if($withinWindow)
                <span class="text-ink/60">Trả hàng khả dụng đến:
                    <b>{{ $completedAt->copy()->addDays($windowDays)->format('d/m/Y H:i') }}</b></span>
                @else
                <span class="text-rose-600">Đơn đã quá hạn {{ $windowDays }} ngày kể từ khi hoàn tất.</span>
                @endif
                @endif
            </div>

            @if($canReturn && Route::has('account.returns.create'))
            <a href="{{ route('account.returns.create', $order) }}"
    ...
    <div class="mt-6 bg-white border border-rose-100 rounded-2xl shadow-sm p-5">
        ...
        @if($endedLabel)
        <div class="mt-3"><span class="inline-flex rounded-full px-2 py-0.5 text-xs {{ $endedPill }}">{{ $endedLabel }}</span></div>
        @endif
    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-rose-100 rounded-2xl shadow-sm p-5 space-y-1">
                ...
                <div class="text-sm">Phương thức:
                    <span class="font-medium">{{ $methodMap[$order->payment_method ?? ''] ?? ($order->payment_method ?? '—') }}</span>
                </div>
                <div class="text-sm">Trạng thái:
                    <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs {{ $payPill }}">{{ $payLabel }}</span>
                </div>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-rose-100">
                     ...
                     text-ink/80 hover:text-white hover:border-transparent hover:bg-gradient-to-r hover:from-rose-600 hover:to-pink-600 hover:shadow-md transition">
                        ...
                        @if($qrCodes->isNotEmpty() && in_array(Str::snake($order->status), ['hoan_tat', 'completed']))
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            <div class="flex items-center gap-3 p-3 bg-white rounded-lg border border-blue-200 shadow-sm">
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <code class="text-xs font-mono break-all">{{ $qr->qr_code }}</code>
                                                </div>
                                                <div class="flex gap-2 flex-shrink-0">
                                                    @if($qr->qr_image_url || $qr->qr_image_path)
                                                    <a href="{{ route('blockchain.qr.download', $qr->id) }}"
                                                    ...
                                                    @endif
                                                    <a href="{{ route('blockchain.verify.qr', $qr->qr_code) }}" target="_blank"
    ...
    <div class="mt-6 bg-white border border-rose-100 rounded-2xl shadow-sm p-5">
        ...
        <div class="text-sm py-2 border-t first:border-0 flex items-start gap-2">
            <span class="text-slate-500 whitespace-nowrap">{{ optional($ev->created_at)->format('d/m/Y H:i') }}</span>
            <span>—</span>
            <div class="flex-1">
                @switch($ev->type)
                @case('status_changed')
                @php $oldSt = data_get($ev->old,'status'); $newSt = data_get($ev->new,'status'); @endphp
                Trạng thái:
                <b>{{ $statusTextMap[$oldSt] ?? Str::title(str_replace('_',' ',$oldSt)) }}</b>
                →
                <b>{{ $statusTextMap[$newSt] ?? Str::title(str_replace('_',' ',$newSt)) }}</b>
                @break

                @case('payment_changed')
                @php $oldPay = data_get($ev->old,'payment_status'); $newPay = data_get($ev->new,'payment_status'); @endphp
                Thanh toán:
                <b>{{ $payTextMap[$oldPay] ?? Str::title(str_replace('_',' ',$oldPay)) }}</b>
                →
                <b>{{ $payTextMap[$newPay] ?? Str::title(str_replace('_',' ',$newPay)) }}</b>
                @break

                @case('tracking_updated')
                Cập nhật mã vận đơn: <b>{{ data_get($ev->new,'tracking_no','') }}</b>
                @break

                @case('note_added')
                Ghi chú: {{ data_get($ev->new,'notes','') }}
                @break

                {{-- Return/Refund events --}}
                @case('return_requested')
                @php $rid = data_get($ev->new,'order_return_id'); $expected = (int) data_get($ev->new,'expected',0); @endphp
                Yêu cầu trả hàng #{{ $rid }} được gửi. Tạm tính hoàn: <b>{{ number_format($expected) }}₫</b>
                @break

                @case('return_approved')
                @php $rid = data_get($ev->new,'order_return_id'); @endphp
                Yêu cầu trả hàng #{{ $rid }} đã được duyệt.
                @break

                @case('return_received')
                @php $rid = data_get($ev->new,'order_return_id'); $final = (int) data_get($ev->new,'final',0); @endphp
                Kho đã nhận hàng trả #{{ $rid }}. Dự kiến hoàn: <b>{{ number_format($final) }}₫</b>
                @break
                ...
                @endphp
                Đã hoàn tiền <b>{{ number_format($amount) }}₫</b> {{ $provider ? 'qua '.$provider : '' }} cho yêu cầu #{{ $rid }}.
                @break

                @default
                {{ $ev->type }}
                @endswitch
                @if(!empty(data_get($ev,'meta.by')))
                <span class="text-ink/50"> · {{ data_get($ev,'meta.by') }}</span>
                @endif
            </div>
        </div>
        @empty
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="w-full max-w-md rounded-2xl bg-white shadow-2xl border border-rose-100 animate-[fadeIn_0.15s_ease-out]">
            <div class="p-5">
                ...
                <form id="cancel-form" method="POST" action="{{ route('account.orders.cancel', ['order' => $order->id]) }}" class="mt-5">
                    @csrf
