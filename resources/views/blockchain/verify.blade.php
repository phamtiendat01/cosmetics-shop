...
@section('title', 'Xác thực sản phẩm - CosmeChain')
        ...
        <div class="max-w-4xl mx-auto">
            ...
            ">
                ...
                <div x-show="activeTab === 'manual'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    <form action="{{ route('blockchain.verify.submit') }}" method="POST" id="verifyForm">
                        @csrf
                        <div class="mb-4">
            ...
            @if(isset($result))
                <div class="bg-white rounded-2xl shadow-lg p-8 result-card">
                    @if(isset($result['authentic']) && $result['authentic'] && ($result['result'] === 'authentic'))
                        {{-- ✅ Authentic Product --}}
                        <div class="text-center mb-8 pb-8 border-b border-gray-200">
                            ...
                            {{-- Warning nếu ở mức khả nghi --}}
                            @if(isset($result['is_suspicious']) && $result['is_suspicious'])
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-amber-800">
                                            QR code này đã được verify <strong>{{ $result['verification_count'] ?? 0 }} lần</strong>.
                                            @if(isset($result['remaining_verifications']) && $result['remaining_verifications'] > 0)
                                                Còn <strong>{{ $result['remaining_verifications'] }} lần</strong> nữa sẽ bị khóa vĩnh viễn.
                                            @endif
                                        </p>
                                ...
                                <div class="bg-gradient-to-br from-brand-50 to-rose-50 rounded-xl p-6 space-y-4">
                                    <div class="grid md:grid-cols-2 gap-4">
                                        <div class="bg-white rounded-lg p-4 shadow-sm">
                                            ...
                                            <div class="font-semibold text-gray-900">{{ $result['certificate']['metadata']['product_name'] ?? 'N/A' }}</div>
                                        </div>
                                        <div class="bg-white rounded-lg p-4 shadow-sm">
                                            <div class="text-sm text-gray-600 mb-1">SKU</div>
                                            <div class="font-semibold text-gray-900">{{ $result['certificate']['metadata']['sku'] ?? 'N/A' }}</div>
                                        </div>
                                        <div class="bg-white rounded-lg p-4 shadow-sm">
                                            <div class="text-sm text-gray-600 mb-1">Thương hiệu</div>
                                            <div class="font-semibold text-gray-900">{{ $result['certificate']['metadata']['brand'] ?? 'N/A' }}</div>
                                        </div>
                                        <div class="bg-white rounded-lg p-4 shadow-sm">
                                            <div class="text-sm text-gray-600 mb-1">Batch Number</div>
                                            <div class="font-semibold text-gray-900">{{ $result['certificate']['metadata']['batch_number'] ?? 'N/A' }}</div>
                                        </div>
                                    </div>
                                    <div class="bg-white rounded-lg p-4 shadow-sm">
                                        <div class="text-sm text-gray-600 mb-2">Certificate Hash</div>
                                        <code class="block text-xs bg-gray-50 p-3 rounded-lg break-all font-mono">{{ $result['certificate']['hash'] }}</code>
                                    </div>
                                    @if(isset($result['certificate']['ipfs_url']))
                                        <a href="{{ $result['certificate']['ipfs_url'] }}" target="_blank"
                        ...
                        @if(isset($result['history']) && count($result['history']) > 0)
                            <div>
                                ...
                                <div class="space-y-3">
                                    @foreach($result['history'] as $index => $movement)
                                        <div class="flex items-start gap-4 p-5 bg-gradient-to-r from-gray-50 to-white rounded-xl hover:shadow-md transition border border-gray-100">
                                            <div class="flex-shrink-0 w-12 h-12 rounded-full bg-gradient-to-br from-brand-100 to-brand-200 flex items-center justify-center text-xl shadow-sm">
                                                @if($movement['type'] === 'manufacture')
                                                    🏭
                                                @elseif($movement['type'] === 'warehouse_in')
                                                    📦
                                                @elseif($movement['type'] === 'warehouse_out')
                                                    🚚
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="text-sm text-gray-600">
                                                    @if($movement['from'])
                                                        <span class="font-medium">{{ $movement['from'] }}</span>
                                                        <i class="fas fa-arrow-right mx-2 text-xs"></i>
                                                    @endif
                                                    <span class="font-medium">{{ $movement['to'] }}</span>
                                                </p>
                                                <p class="text-xs text-gray-500 mt-2">
                                                    <i class="far fa-clock mr-1"></i>{{ $movement['date'] }}
                                                </p>
                                            </div>
                                        </div>
                                    @endforeach
                    ...
                    @else
                        ...
                        <div class="text-center py-8">
                            <div class="inline-flex items-center justify-center w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-red-400 to-red-600 mb-6 shadow-lg overflow-hidden">
                                @if(isset($result['result']) && $result['result'] === 'suspicious')
                            ...
                            <h2 class="text-3xl md:text-4xl font-bold text-red-600 mb-3">
                                {{ $result['message'] ?? 'QR code không tồn tại' }}
                            </h2>
...
window.startScanner = function() {
    ...
    const qrCodeSuccessCallback = (decodedText, decodedResult) => {
        ...
        form.method = 'POST';
        form.action = '{{ route("blockchain.verify.submit") }}';
