    ...
    <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-card
        ...
        <div class="border-t border-rose-100 p-4 space-y-3">
            ...
            <div class="flex items-center gap-2">
                <a href="<?php echo e(route('checkout.index')); ?>" class="flex-1 px-4 py-3 bg-brand-600 text-white rounded-xl text-center hover:bg-brand-700">
                    Thanh toán
                </a>
...
<script>
    function cartDrawer() {
        return {
            ...
            async refresh() {
                try {
                    const r = await fetch("<?php echo e(route('cart.json')); ?>", {
                        headers: {
            ...
            async updateQty(it, qty) {
                try {
                    const r = await fetch(`<?php echo e(url('/cart')); ?>/${encodeURIComponent(it.key)}`, {
                        method: 'PATCH',
                        headers: {
            ...
            async clearAll() {
                ...
                try {
                    const r = await fetch(`<?php echo e(url('/cart')); ?>`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json'
...
</script><?php /**PATH C:\xampp\htdocs\cosmetics-shop\resources\views/components/cart-drawer.blade.php ENDPATH**/ ?>