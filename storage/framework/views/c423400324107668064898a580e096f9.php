...
<?php $__env->startSection('title', 'CosmeBot - Chủ đề trò chuyện'); ?>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white border border-slate-200 rounded-xl p-4">
        <div class="text-2xl font-bold text-rose-600 mb-1"><?php echo e($intents->total()); ?></div>
        <div class="text-sm text-slate-600">Tổng chủ đề</div>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl p-4">
        <div class="text-2xl font-bold text-green-600 mb-1"><?php echo e($intents->where('is_active', true)->count()); ?></div>
        <div class="text-sm text-slate-600">Đang hoạt động</div>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl p-4">
        <div class="text-2xl font-bold text-blue-600 mb-1"><?php echo e($intents->where('is_active', false)->count()); ?></div>
        <div class="text-sm text-slate-600">Đã tắt</div>
    </div>
    <div class="bg-white border border-slate-200 rounded-xl p-4">
        <div class="text-2xl font-bold text-purple-600 mb-1"><?php echo e($intents->max('priority') ?? 0); ?></div>
    ...
    <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-200">
            ...
            <tr class="hover:bg-slate-50">
                <td class="px-6 py-4">
                    <div class="font-medium text-slate-900"><?php echo e($intent->display_name); ?></div>
                    <div class="text-xs text-slate-500 font-mono mt-1"><?php echo e($intent->name); ?></div>
                </td>
                <td class="px-6 py-4">
                    <div class="text-sm text-slate-700 max-w-md"><?php echo e(Str::limit($intent->description ?? 'Chưa có mô tả', 80)); ?></div>
                </td>
                <td class="px-6 py-4">
                    <span class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs font-semibold">
                        <?php echo e($intent->priority); ?>
                ...
                <td class="px-6 py-4">
                    <button onclick="editIntent(<?php echo json_encode($intent, 15, 512) ?>)" 
...
<div id="intentModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <form method="POST" action="<?php echo e(route('admin.bot.intents.store')); ?>" class="p-6">
            <?php echo csrf_field(); ?>
