<?php $__env->startSection('title', 'Tổng quan tài khoản'); ?>
...
<section class="dashboard-ambient max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
            <h1 class="text-3xl md:text-4xl font-extrabold t-gradient leading-tight">
                Xin chào, <?php echo e($user->name ?? $user->email); ?>
        ...
        <div class="glass rounded-2xl p-4 lift">
            <div class="flex items-center justify-between">
                <div class="w-9 h-9 rounded-xl flex items-center justify-center text-white"
                    ...
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path d="<?php echo e($k['icon']); ?>" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </div>
                <span class="text-xs text-slate-500"><?php echo e($k['hint']); ?></span>
            </div>
            <div class="text-xs uppercase tracking-wide text-slate-500 mt-2"><?php echo e($k['label']); ?></div>
            <div class="text-[28px] font-extrabold mt-1"><?php echo e(is_numeric($k['val'])?number_format($k['val']):$k['val']); ?></div>
        </div>
    ...
    <div class="grid lg:grid-cols-3 gap-4 mt-4">
        <div class="lg:col-span-2 bg-white rounded-2xl glow-border p-4 lift">
            <div class="flex items-center justify-between mb-2">
                <div class="font-semibold"><?php echo e($charts['topTitle'] ?? 'Sản phẩm mua nhiều'); ?></div>
                <div class="text-xs text-slate-500">Top 5</div>
            </div>
            <?php if(empty($charts['topLabels'])): ?>
            <div class="text-sm text-slate-500">Chưa có dữ liệu.</div>
            <?php else: ?>
        ...
        <div class="glass rounded-2xl p-4 lift">
            ...
            <div class="grid grid-cols-2 gap-3">
                <a class="shine rounded-xl border px-3 py-3 hover:bg-white" href="<?php echo e(route('account.profile')); ?>">
                    <div class="font-medium text-sm">Trang cá nhân</div>
                    <div class="text-slate-500 text-xs">Cập nhật thông tin</div>
                </a>
                <a class="shine rounded-xl border px-3 py-3 hover:bg-white" href="<?php echo e(route('account.addresses.index')); ?>">
                    <div class="font-medium text-sm">Địa chỉ</div>
                    <div class="text-slate-500 text-xs">Quản lý giao hàng</div>
                </a>
                <a class="shine rounded-xl border px-3 py-3 hover:bg-white" href="<?php echo e(route('account.coupons')); ?>">
                    <div class="font-medium text-sm">Mã giảm giá</div>
                    <div class="text-slate-500 text-xs">Đã dùng: <?php echo e(number_format($redeemedCoupons)); ?></div>
                </a>
                <a class="shine rounded-xl border px-3 py-3 hover:bg-white" href="<?php echo e(route('account.reviews')); ?>">
                    <div class="font-medium text-sm">Đánh giá</div>
                    <div class="text-slate-500 text-xs">Đã viết: <?php echo e(number_format($reviewCount)); ?></div>
                </a>
                <a class="shine rounded-xl border px-3 py-3 hover:bg-white" href="<?php echo e(route('account.wishlist')); ?>">
                    <div class="font-medium text-sm">Yêu thích</div>
                    <div class="text-slate-500 text-xs">SP: <?php echo e(number_format($wishlistCount)); ?></div>
                </a>
                <a class="shine rounded-xl border px-3 py-3 hover:bg-white" href="<?php echo e(route('account.orders.index')); ?>">
                    <div class="font-medium text-sm">Đơn hàng</div>
                    <div class="text-slate-500 text-xs">Xem lịch sử</div>
                </a>
                <a class="shine rounded-xl border px-3 py-3 hover:bg-white bg-gradient-to-br from-brand-50 to-rose-50 border-brand-200" href="<?php echo e(route('blockchain.verify')); ?>">
    ...
    <div class="bg-white rounded-2xl glow-border p-4 mt-6 lift">
        <div class="flex items-center justify-between">
            ...
            <a href="<?php echo e(route('account.addresses.index')); ?>" class="text-sm text-rose-600 hover:underline">Quản lý địa chỉ</a>
        </div>
        <div class="grid md:grid-cols-2 gap-4 mt-3">
            ...
            <div class="glass rounded-xl p-3">
                ...
                <div class="text-slate-600 text-sm leading-6">
                    ...
                    <?php echo e($shipping['line1'] ?? ''); ?> <?php echo e($shipping['line2'] ?? ''); ?><br>
                    <?php echo e($shipping['ward'] ?? ''); ?><?php echo e(!empty($shipping['district']) ? ', '.$shipping['district'] : ''); ?><?php echo e(!empty($shipping['province']) ? ', '.$shipping['province'] : ''); ?>
            ...
            <div class="glass rounded-xl p-3">
                ...
                <div class="text-slate-600 text-sm leading-6">
                    <?php echo e($billing['name'] ?? ''); ?><br>
                    <?php echo e($billing['phone'] ?? ''); ?><br>
                    <?php echo e($billing['line1'] ?? ''); ?> <?php echo e($billing['line2'] ?? ''); ?><br>
                    <?php echo e($billing['ward'] ?? ''); ?><?php echo e(!empty($billing['district']) ? ', '.$billing['district'] : ''); ?><?php echo e(!empty($billing['province']) ? ', '.$billing['province'] : ''); ?>
    ...
    <div class="bg-white rounded-2xl glow-border mt-6 lift">
        <div class="p-4 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('account.orders.index')); ?>" class="text-sm text-rose-600 hover:underline">Xem tất cả</a>
        </div>
        ...
        <div class="divide-y">
            ...
            <a href="<?php echo e(route('account.orders.show', $o->id)); ?>" class="flex items-center justify-between p-4 hover:bg-rose-50/50">
                <div>
                    <div class="font-medium">#<?php echo e($o->code); ?></div>
                    <div class="text-xs text-slate-500"><?php echo e(optional($o->created_at)->format('d/m/Y H:i')); ?></div>
                </div>
                <div class="text-right">
                    <div class="text-sm"><?php echo e(ucfirst(str_replace('_',' ', $o->status))); ?></div>
                    <div class="text-xs text-slate-500"><?php echo e(ucfirst(str_replace('_',' ', $o->payment_status))); ?></div>
                    <div class="font-semibold mt-1">₫<?php echo e(number_format($o->grand_total)); ?></div>
                </div>
            </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
...
<script>
    ...
    const C = {
        labels: <?php echo json_encode($charts['labels'] ?? [], 15, 512) ?>,
        seriesOrders: <?php echo json_encode($charts['seriesOrders'] ?? [], 15, 512) ?>,
        seriesAmount: <?php echo json_encode($charts['seriesAmount'] ?? [], 15, 512) ?>,
        pieLabels: <?php echo json_encode($charts['pieLabels'] ?? [], 15, 512) ?>,
        pieValues: <?php echo json_encode($charts['pieValues'] ?? [], 15, 512) ?>,
        topLabels: <?php echo json_encode($charts['topLabels'] ?? [], 15, 512) ?>,
        topValues: <?php echo json_encode($charts['topValues'] ?? [], 15, 512) ?>
    };
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\cosmetics-shop\resources\views/account/dashboard.blade.php ENDPATH**/ ?>