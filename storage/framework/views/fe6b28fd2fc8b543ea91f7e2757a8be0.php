...
<div class="max-w-6xl mx-auto px-4 py-6">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-6">
        <?php $__empty_1 = true; $__currentLoopData = $addresses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <?php $est = $estimates[$a->id] ?? ['km'=>null,'fee'=>null]; ?>
        ...
        <div class="bg-white rounded-2xl border border-rose-100 shadow-card p-5 hover:shadow-lg hover:border-rose-200 transition">
            <div class="flex items-start justify-between">
                <div class="text-lg font-semibold"><?php echo e($a->name); ?></div>
            ...
            <div class="mt-2 text-sm text-ink">
                <div class="font-medium"><?php echo e($a->phone); ?></div>
                <div><?php echo e($a->line1); ?> <?php echo e($a->line2); ?></div>
                <div><?php echo e($a->ward); ?><?php echo e($a->district ? ', '.$a->district : ''); ?><?php echo e($a->province ? ', '.$a->province : ''); ?></div>
            </div>

            <div class="mt-3 text-xs text-ink/70">
                <?php if(is_null($est['km'])): ?>
                <div class="flex items-center gap-1"><i class="fa-regular fa-circle-question"></i> Chưa có toạ độ – sửa địa chỉ để ghim.</div>
                <?php else: ?>
                <div>Khoảng cách: <b><?php echo e(number_format($est['km'],2)); ?> km</b></div>
                <div>Phí ship ước tính: <b><?php echo e(($est['fee'] ?? 0)==0 ? 'Miễn phí' : '₫'.number_format($est['fee'])); ?></b></div>
                <?php endif; ?>
            </div>
            ...
            <div class="mt-4 grid grid-cols-2 gap-3">
                <button type="button"
                    ...
                    data-update="<?php echo e($addrBase.'/'.$a->id); ?>"
                    data-address='<?php echo json_encode($a, 15, 512) ?>'>
                    Sửa
                </button>

                <button type="button"
                    class="btn-delete rounded-lg border border-rose-200 px-3 py-2 text-sm hover:bg-rose-50"
                    data-action="<?php echo e($destroyUrl($a->id)); ?>">
                    Xoá
                </button>
            </div>
            ...
            <div class="mt-3 grid grid-cols-2 gap-3">
                <?php if (! ($a->is_default_shipping)): ?>
                <form method="POST" action="<?php echo e($defaultUrl($a->id)); ?>">
                    <?php echo csrf_field(); ?>
...
<div id="addrModal" class="fixed inset-0 z-[999] hidden items-center justify-center bg-black/40">
    <div class="w-[780px] max-w-[95vw] bg-white rounded-2xl shadow-card" role="dialog" aria-modal="true">
        <form id="addrForm" method="POST" action="<?php echo e($addrStore); ?>" class="p-6 space-y-5">
            <?php echo csrf_field(); ?>
            <input type="hidden" id="addrMethod" value=""> 
...
<script>
    (function() {
        ...
        function openCreate() {
            ...
            form.action = <?php echo json_encode($addrStore, 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\cosmetics-shop\resources\views/account/addresses/index.blade.php ENDPATH**/ ?>