...
<?php $__env->startSection('title','Yêu cầu trả hàng'); ?>
...
<div class="max-w-5xl mx-auto px-4 py-6">
    ...
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold">Yêu cầu trả hàng — Đơn #<?php echo e($order->code); ?></h1>
            <div class="text-sm text-ink/60 mt-1">Đặt lúc <?php echo e(optional($order->created_at)->format('d/m/Y H:i')); ?></div>
        </div>
        <a href="<?php echo e(route('account.orders.show', $order)); ?>" class="btn btn-outline">← Quay lại đơn</a>
    </div>
    ...
    <div class="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900 text-sm">
        ...
        <ul class="list-disc pl-5 space-y-1">
            ...
            <li>Thời hạn yêu cầu: trong vòng <b><?php echo e($windowDays); ?></b> ngày kể từ khi hoàn tất.</li>
            <?php if($completedAt): ?>
            <li>Hạn cuối: <b><?php echo e($completedAt->copy()->addDays($windowDays)->format('d/m/Y H:i')); ?></b></li>
            <?php endif; ?>
        </ul>
        <?php if (! ($withinWindow)): ?>
        <div class="mt-2 rounded-xl border border-rose-200 bg-rose-50 text-rose-800 p-3">
            Đơn đã quá hạn <?php echo e($windowDays); ?> ngày kể từ khi hoàn tất nên không thể tạo yêu cầu.
        </div>
        <?php endif; ?>
    </div>

    
    <form id="return-form" method="POST" action="<?php echo e(route('account.returns.store', $order)); ?>" class="mt-6 grid lg:grid-cols-3 gap-6">
        <?php echo csrf_field(); ?>
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white border border-rose-100 rounded-2xl shadow-sm p-5">
                ...
                <div class="grid sm:grid-cols-2 gap-2 mb-3">
                    <?php $commonReasons = ['Sản phẩm không phù hợp','Thiếu/nhầm hàng','Lỗi seal/tem','Hư hỏng do vận chuyển','Khác']; ?>
                    <?php $__currentLoopData = $commonReasons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reason): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <label class="inline-flex items-center gap-2 rounded-xl border px-3 py-2 cursor-pointer hover:bg-rose-50">
                        <input type="radio" name="reason_choice" value="<?php echo e($reason); ?>" class="accent-rose-600">
                        <span class="text-sm"><?php echo e($reason); ?></span>
                    </label>
                        ...
                        <tr class="hover:bg-rose-50/40">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <?php endif; ?>
                                    <div class="min-w-0">
                                        <div class="font-medium truncate"><?php echo e($it->product_name_snapshot ?? optional($it->product)->name ?? 'Sản phẩm'); ?></div>
                                        <?php if($variantLabel): ?>
                                        <div class="text-xs text-ink/60"><?php echo e($variantLabel); ?></div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center"><?php echo e($it->qty); ?></td>
                            <td class="px-4 py-3 text-right">₫<?php echo e(number_format($unit)); ?></td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <input class="qty-input w-16 border rounded px-2 py-1 text-center"
                                        type="number" min="0" max="<?php echo e($it->qty); ?>"
                                        name="items[<?php echo e($loop->index); ?>][qty]"
                                        value="<?php echo e(old('items.'.$loop->index.'.qty', 0)); ?>"
                                        data-price="<?php echo e($unit); ?>">
                                    <button class="qty-btn w-8 h-8 rounded border hover:bg-rose-50" type="button" data-act="inc">＋</button>
                                </div>
                                <div class="text-[11px] text-ink/50 mt-1">Tối đa: <?php echo e($it->qty); ?></div>
                            </td>
                        </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </tbody>
        ...
        <aside class="space-y-4">
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-2 text-xs text-ink/50">
                            Số dư ví hiện tại: <b>₫<?php echo e(number_format($walletBalance)); ?></b>.
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end gap-2">
                <a href="<?php echo e(route('account.orders.show',$order)); ?>" class="btn btn-outline">Hủy</a>
                <button id="btn-open-confirm" type="button"
                    class="btn btn-primary <?php echo e($withinWindow ? '' : 'opacity-50 cursor-not-allowed'); ?>"
                    <?php echo e($withinWindow ? '' : 'disabled'); ?>>
                    Gửi yêu cầu
                </button>
            </div>
        </aside>
...
<script>
    (() => {
        ...
        const withinWindow = <?php echo json_encode($withinWindow, 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\cosmetics-shop\resources\views/account/returns/create.blade.php ENDPATH**/ ?>